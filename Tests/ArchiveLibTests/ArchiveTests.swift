...
class ArchiveTests: XCTestCase {
    ...
    func testDocumentRemove() {
        ...
        // calculate
        guard let taggedDocument = archive.get(scope: .all, searchterms: [], status: .tagged).first else { XCTFail(); return }
        guard let untaggedDocument = archive.get(scope: .all, searchterms: [], status: .untagged).first else { XCTFail(); return }
    ...
    func testDocumentUpdate1() {
        ...
        // calculate
        guard let taggedDocument = archive.get(scope: .all, searchterms: ["aaa"], status: .tagged).first else { XCTFail(); return }
        guard let untaggedDocument = archive.get(scope: .all, searchterms: ["ddd"], status: .untagged).first else { XCTFail(); return }
        ...
        XCTAssertEqual(archive.get(scope: .all, searchterms: [], status: .untagged).count, 1)
        guard let newTaggedDocument = archive.get(scope: .all, searchterms: ["aaa"], status: .tagged).first else { XCTFail(); return }
        guard let newUntaggedDocument = archive.get(scope: .all, searchterms: ["ddd"], status: .untagged).first else { XCTFail(); return }
    ...
    func testGetAvailableTags() {
        ...
        // setup
        let _ = archive.add("tagTest1")
        let _ = archive.add("tagTest2", count: 2)
        let _ = archive.add("tagTest3", count: 3)
