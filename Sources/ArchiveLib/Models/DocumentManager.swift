...
public protocol DocumentManagerHandling: AnyObject {
    ...
    func add(from path: URL, size: Int64?, downloadStatus: DownloadStatus, status: TaggingStatus, parseContent: Bool)
    func remove(_ removableDocuments: Set<Document>)
    func removeAll(_ status: TaggingStatus)
    func update(_ document: Document)
    func update(from path: URL, size: Int64?, downloadStatus: DownloadStatus, status: TaggingStatus, parseContent: Bool) -> Document
